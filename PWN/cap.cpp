...
// Function to save an XImage as a PNG file using libpng
...
// Function to capture a window's content and save it as a PNG
...
// Recursively list all top-level windows and capture them
void list_and_capture_windows(Display *display, Window root) {
    ...
    if (XQueryTree(display, root, &root, &parent, &children, &nchildren)) {
        for (unsigned int i = 0; i < nchildren; i++) {
            ...
            if (children[i] != root) {
                // Create a unique filename for each captured window
                char filename[256];
                snprintf(filename, sizeof(filename), "window_0x%lx.png", children[i]);

                // Capture this window's content
                capture_window(display, children[i], filename);
            }
        }
        if (children) {
            XFree(children);
...
int main() {
    ...
    // Capture all top-level windows
    list_and_capture_windows(display, root);
